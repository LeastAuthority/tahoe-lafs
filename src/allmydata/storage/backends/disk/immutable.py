...
from allmydata.storage.common import si_b2a, CorruptShareError, UnknownImmutableContainerVersionError, \
     DataTooLargeError
    ...
    def __init__(self, home, storage_index, shnum, finalhome=None, allocated_data_length=None):
        ...
        if self._finalhome is not None:
            ...
            try:
                ...
                raise CorruptShareError(shnum, "invalid immutable share header for shnum %d: %s" % (shnum, e))
            finally:
                f.close()
            if version != 1:
        ...
        if self._data_length < 0:
            raise CorruptShareError("calculated data length for shnum %d is %d" % (shnum, self._data_length))
