...
def invite(options):
    ...
    try:
        ...
        try:
            with open(join(basedir, "private", "introducer.furl"), "r") as f:
                introducer_furl = f.read().strip()
        except IOError:
            print >>err, "Can't find introducer fURL in tahoe.cfg nor " + \
                "{}/private/introducer.furl".format(basedir)
            raise SystemExit(1)
    ...
    wh = wormhole.create(
        appid=options.parent['wormhole-appid'],
        relay_url=relay_url,
        reactor=reactor,
    )
